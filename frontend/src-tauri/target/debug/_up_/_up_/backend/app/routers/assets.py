...
# ==========================================
# 0. 视觉资产数字化（图片 -> JSON 一致性层）
# ==========================================
@router.post("/ingest", response_model=schemas.VisualProfileLibrary)
def ingest_assets_to_visual_profile(
    payload: schemas.VisualAssetIngestRequest,
    db: Session = Depends(get_db),
):
    """
    无 LLM 保底实现：通过 Pillow/metadata 提取，生成可校验的 VisualProfileLibrary。
    后续接 LLM 时，只需要替换 service 层，不改 API 契约。
    """
    try:
        return ingest_visual_assets(payload=payload, db=db, data_root=DATA_ROOT)
