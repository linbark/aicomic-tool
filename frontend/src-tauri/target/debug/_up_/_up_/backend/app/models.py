...
# =======================
# 4. LLM Provider 配置 (P3+)
# =======================

class LLMProviderConfig(Base):
    __tablename__ = "llm_provider_configs"
    id = Column(Integer, primary_key=True, index=True)
    provider_name = Column(String, unique=True, index=True, nullable=False)  # "openai", "claude", "gemini" 等
    is_active = Column(Boolean, default=False)  # 是否激活（同一时间只有一个 active）
    config_data = Column(JSON, nullable=False)  # 存储 provider 特定配置（如 api_key, model, base_url 等）
    created_at = Column(DateTime(timezone=True), server_default=func.now())
